...
async function loadUserCorrectCount() {
  try {
    ...
    totalCorrect.value = res.data.correctAWS.length
  } catch (_) {
...
async function submitAnswer() {
  ...
  if (isCorrect) {
    try {
      await axios.post('/api/aws/mark-correct', {
        awsId: currentQuestion.value._id
      })
